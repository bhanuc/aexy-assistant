...
Fork code under `assistant/src/live/`, on only when `assistant-desktop` and `aexy-live-view` are both on in a container; contracts are in `docs/aexy/AGENT_LIVE_VIEW_CONTRACTS.md`.

**Watch stream.** `/v1/watch/stream` is shared with upstream's narration capture above; the daemon routes an upgrade to the live view only when the gateway's dial carries `x-vellum-stream-scope`, and reads the viewer (`x-vellum-viewer-id`, `-role`, `-scope`, `-live-session-id`, `-aexy-developer-id`, `-display-name`, `-conversation-id`) from the headers the gateway attested (C1.4). The upgrade gate is the shared `upgradeRuntimeStream` one; the feature gate closes `4008` and a missing or malformed viewer `4003` in the open handler. `LiveWatchHub` (`watch-hub.ts`) holds every viewer: one process subscription to the event hub fans each event out as `{type:"event", seq, emittedAt, conversationId, event}` to viewers of that conversation, with `assistant_thinking_delta` only for `owner|manager|admin` and `tool_result.imageData`/`imageDataList` stripped. A viewer's conversation is its token's, else the active workspace-task conversation, else the most recent background one; followers get a fresh `hello` when a claim's turn starts. `DesktopScreencaster` (`screencast.ts`) runs CDP `Page.startScreencast` (jpeg, quality 60, 1280x800) on its own DevTools connection to the page the agent's cdp-inspect client last attached to, only while a frame-receiving (non-`chat`) viewer is connected, and rate-limits by delaying `Page.screencastFrameAck` to the highest `quality.maxFps` any viewer asked for (default 4). Each frame is a JSON `frame` header then one binary JPEG; a viewer whose socket has more than 256 KiB unsent keeps only the newest skipped frame and receives it on `drain`. While anyone watches, the platform's `record-activity` is called every 30 s and the desktop is held. `GET /v1/watch/snapshot` (gateway principals only) serves the newest frame with `x-frame-ts` / `x-frame-url`, takes a fresh still when nothing is casting and the desktop is already up, and answers `204` when there is no picture.

**Control.** `POST /v1/live/control` (gateway principals only; `live-routes.ts` → `LiveControl` in `live-control.ts`, wired to the daemon in `live-control-runtime.ts`) takes C4.2's commands. State lives in `control-state.ts` and every change is broadcast as `control_state` on the watch stream. `pause` is honoured at step boundaries: `AgentLoop.run` awaits `waitWhileLivePaused` before each model call and before each tool batch, so the tool in flight finishes and the next step waits; an abort ends the wait. `instruct` queues `Instruction from {name} ({role}): {text}` on the target conversation with `queueWhenIdle` (a running turn takes it at its next checkpoint handoff, an idle one starts a turn). `stop` marks the claimed card settled, releases it with `Stopped by {name}`, and aborts the turn. `acquire_control` pauses and grants one lease; while it is held `ToolExecutor.execute` refuses every `browser_*` and `computer_use*` tool, and `/v1/desktop/stream` admits only the holder (matched on `x-vellum-aexy-developer-id`, `4013` otherwise, `4003` for a non-control scope; with no lease a live-session dial is refused and the guardian path is upstream's). A closed desktop socket starts a 120 s clock rather than ending the lease. `release_control` ends it, resumes, and appends `{name} handed back control ({outcome}): {note}` with a fresh screenshot and a warning that the page must be re-checked; `signin_decision` answers `501 not_implemented`.

**Takeover requests (D5).** Under the live view a CAPTCHA that survives the five-second auto-resolve wait on the cdp-inspect backend no longer goes through `startHandoff`'s URL-change wait. `requestTakeover` (`takeover.ts`) posts a C8 `kind:"takeover"` ask (`origin_ref` = the takeover id, no questions, `takeover:{reason, what_to_do, deadline_seconds, page}`) through `workspace-inbox.ts`, puts `{id, reason}` on the control state so watchers see it, and waits for a `release_control` naming it (or the lease taken for it expiring), the workspace reporting it `expired` or `answered`, the deadline (30 minutes by default), or an abort. With no inbox and nobody watching it returns at once as unavailable. The tool result names who handed back, how and with what note, and the caller re-runs the CAPTCHA detector before saying the challenge is gone.

**Per-person chat (C6).** `POST /v1/messages` reads `x-vellum-acting-user-id`, `-name`, `-role` and `x-vellum-acting-aexy-developer-id` (`acting-user.ts`), which only the gateway sets and only for someone on the access list who is not the guardian. Such a person's new conversation is keyed `aexy-user:<platform user id>:…` in `conversation_keys`, which is the durable record of whose thread it is; naming a conversation id that is not theirs is a `404`. Each send records who is speaking on the conversation (the guardian's clears it), and the `unified-turn-context` injector starts that turn's context with `You are talking with {name} ({role}).`
